...
UdpSocket::UdpSocket() : socket_fd_(-1)
{
    try
    {
        ...
        if ((socket_fd_ = socket(AF_UNSPEC, SOCK_DGRAM,
                    0)) == -1) {
...
void UdpSocket::send_to(const std::string& data, const std::string& ip, const std::string& port)
{
    struct addrinfo hints{};
    hints.ai_family = AF_UNSPEC;
    hints.ai_socktype = SOCK_DGRAM; // Datagram for UDP
    ...
    struct addrinfo *servinfo = nullptr;  // will point to the results
    if (int status = getaddrinfo(ip.c_str(), port.c_str(), &hints, &servinfo);status != 0)
    {
        throw std::runtime_error(gai_strerror(status));
    }
...
std::string UdpSocket::receive_from(std::string& ip, std::string& port)
{
  ...
  if ((bytes_received = recvfrom(socket_fd_, buffer, MAXBUFLEN-1 , 0,
      reinterpret_cast<struct sockaddr *>(&their_addr), &addr_len)) == -1) {
      throw std::runtime_error("recvfrom failed");
      }

  printf("listener: got packet from %s\n",
      inet_ntop(their_addr.ss_family,
                sussy_socket::net::get_in_addr(reinterpret_cast<struct sockaddr *>(&their_addr)),
          s, sizeof s));
